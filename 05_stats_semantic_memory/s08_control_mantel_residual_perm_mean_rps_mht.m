...
load(fullfile(deriv_dir, ['rps_spc_' demean_id], 'group', ...
    ['resid_dist_cross_subj' num2str(n_subjs) '_rps_spc_pairwise_' demean_id '_perm' ...
    num2str(n_perms) 'mean.mat']), 'resid_rps_mean_perm') % structure
% region * time * subj * subj
n_regions = size(resid_rps_mean_perm.age_dist, 1);
n_times = size(resid_rps_mean_perm.age_dist, 2);
...
for imem = 1:2
    ...
    for iconfs = 1:length(confs_list)
        ...
        neural_mat = resid_rps_mean_perm.(conf_id);
    ...
    save(fullfile(res_dir, ['res_parmantel_mht-word-' memory_id '_rps-spc-word_perm' ...
        num2str(n_perms) 'mean_subjs' num2str(n_subjs) '_' demean_id '1w.mat']), ...
        'pearson_r', 'pvalue')
    toc
end
