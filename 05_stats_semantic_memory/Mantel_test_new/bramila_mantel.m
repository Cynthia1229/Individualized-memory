...
% see reference: https://users.aalto.fi/~eglerean/permutations.html
% cite as: Glerean, E., Pan, R.K., Salmi, J., Kujala, R., Lahnakoski, J.M., Roine, U., Nummenmaa, L., Lepp?m?ki, S., Nieminen-von Wendt, T., Tani, P., Saram?ki, J., Sams, M., J??skel?inen, I.P. (in press) Reorganization of functionally connected brain subnetworks in high-functioning autism Human Brain Mapping. doi:10.1002/hbm.23084
    ...
    % modified by Jintao Sheng 20220108
    ...
    xtem = xSim; xtem(isnan(xtem)) = 0; % remove nan for checking symmetric
    ytem = modelMat; ytem(isnan(ytem)) = 0; % remove nan for checking symmetric
	temp=xtem-xtem';
    ...
    out=corr(xSim(ids),modelMat(ids),'type',type, 'rows', 'pairwise');
	
    surro=zeros(iter,1);
    for i=1:iter
		pe=randperm(size(xSim,1));
		temp=xSim(pe,pe);
		surro(i)=corr(temp(ids),modelMat(ids),'type',type, 'rows', 'pairwise');
   	end
