function s01_cluster_permutation_test_parmantel_multiprcs(shuffle_num)
    ...
    for iconfs = 1:length(confs_list)
        ...
        for ishuffle = 1:shuffle_num
            ...
            for iregion = 1:n_regions % only permute original significant regions
                parfor itime = 1:n_times
                    mtx1 = squeeze(neural_mat(iregion,itime,:,:));
                    [mrho, mpval] = bramila_mantel(mtx1, mht_temp, 1000, 'pearson');
                    Corr_beh_STPS_MHT_tem_rho(iregion, itime) = atanh(mrho); %%%% transformed to fisherZ %%%%
        ...
        out_dir = fullfile(res_dir, ['parmantel_dprime_perm_' memory_id '_multiprcs']);
        
        if ~ exist(out_dir, 'dir')
            mkdir(out_dir)
        end
    end
    save(fullfile(out_dir, [out_name '_zrsum_max_all' num2str(shuffle_num, '%04d') '.mat']), ...
        'rsum_max_mht')
end
